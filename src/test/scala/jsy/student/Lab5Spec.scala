...
class Lab5Spec(lab5: Lab5Like) extends FlatSpec {
  ...
  "TypeFunction" should "perform TypeFunction" in {
    assertResult(TFunction(List(("n",MTyp(MConst,TNumber))), TNumber)){
      typeof(empty, Function(Some("f"),List(("l",MTyp(MConst,TNumber))),Some(TNumber),If(Binary(Eq,Var("l"),N(3)),N(2),Call(Var("f"),List(Binary(Minus,N(8),N(2)))))))
    }
  }
